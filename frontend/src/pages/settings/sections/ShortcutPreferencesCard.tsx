...
function ShortcutBindingPill({ binding, platform }: { binding: string; platform: string }) {
    return (
        <span className="inline-flex flex-wrap items-center gap-1 rounded-full border border-warm-200 bg-white px-2.5 py-1.5 text-xs font-medium text-warm-600 shadow-sm dark:border-warm-700 dark:bg-warm-900/70 dark:text-warm-300">
            {formatShortcutBinding(binding, platform).map((key) => (
                <kbd
                    key={`${binding}-${key}`}
                    className="rounded-md border border-warm-200 bg-warm-50 px-1.5 py-0.5 font-mono text-[11px] text-warm-600 dark:border-warm-700 dark:bg-warm-800 dark:text-warm-300"
                >
                    {key}
                </kbd>
            ))}
...
export default function ShortcutPreferencesCard() {
    ...
    return (
        <div className="glass-card p-6">
            ...
            {!isElectron ? (
                <div className="mt-4 rounded-2xl border border-warm-200 bg-warm-50 px-4 py-3 text-sm text-warm-500 dark:border-warm-700 dark:bg-warm-900/40 dark:text-warm-400">
                    {t('settings.general.shortcuts.desktopOnlyHint', 'This shortcut is only available in the Electron desktop app.')}
                </div>
            ) : (
                <>
                    <div className="mt-5 rounded-2xl border border-warm-200/80 bg-white/80 p-4 dark:border-warm-700/70 dark:bg-warm-900/40">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="min-w-0">
                                ...
                                <div className="mt-3 flex flex-wrap items-center gap-2">
                                    <span className="text-xs font-semibold uppercase tracking-[0.18em] text-warm-400 dark:text-warm-500">
                                        {t('settings.general.shortcuts.current', 'Current')}
                                    </span>
                                    {currentBinding ? (
                                        <ShortcutBindingPill binding={currentBinding} platform={platform} />
                                    ) : (
                                        <span className="rounded-full border border-dashed border-warm-300 px-3 py-1 text-xs font-medium text-warm-400 dark:border-warm-700 dark:text-warm-500">
                                ...
                                {isRecording ? (
                                    <>
                                        ...
                                        <button
                                            ...
                                            className="inline-flex items-center justify-center rounded-xl border border-warm-200 px-3 py-2 text-sm font-medium text-warm-600 transition-colors hover:border-warm-300 hover:bg-warm-50 dark:border-warm-700 dark:text-warm-300 dark:hover:border-warm-600 dark:hover:bg-warm-800"
                                        >
                                    ...
                                    <>
                                        ...
                                        <button
                                            ...
                                            onClick={handleReset}
                                            disabled={isBusy}
                                            className="inline-flex items-center justify-center gap-2 rounded-xl border border-warm-200 px-3 py-2 text-sm font-medium text-warm-600 transition-colors hover:border-warm-300 hover:bg-warm-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-warm-700 dark:text-warm-300 dark:hover:border-warm-600 dark:hover:bg-warm-800"
                                        >
                                            <RotateCcw size={16} />
                    ...
                    <p className="mt-3 text-xs text-warm-400 dark:text-warm-500">
